...
# The benchmark is strictly sequential — one LLM call per piece placement — so
# there is no throughput benefit to a large batch. vLLM 0.19.1 warms up the
# sampler with max_num_seqs dummy requests (gpu_model_runner._dummy_sampler_run);
# at the default 1024 that allocation is ~1 GiB and OOMed on startup.
MAX_NUM_SEQS="${MAX_NUM_SEQS:-8}"
DTYPE="${DTYPE:-float16}"
QUANTIZATION="${QUANTIZATION-awq_marlin}"
KV_CACHE_DTYPE="${KV_CACHE_DTYPE:-fp8_e4m3}"
MAX_NUM_BATCHED_TOKENS="${MAX_NUM_BATCHED_TOKENS:-8192}"
# 0.90, not 0.9636: at 0.9636 only ~675 MiB was left free, and vLLM 0.19.1's
# post-load sampler warmup then failed to allocate. 0.90 leaves several GiB of
# headroom, which this workload does not need for KV cache anyway.
...
#   2. The main repo's own .venv — the working local venv on this node, and the
#      only one visible inside an experiment sandbox (worktrees have no .venv of
#      their own, and the sandbox bind-mounts the repo tree but not $HOME).
#   3. $SHARED_VENV_DIR, if exported — an out-of-repo venv such as a shared
#      cluster venv or a Slurm base-venv. Deliberately not defaulted: no
#      site-specific path is hardcoded here, so export it per host if needed.
if [ ! -e "$ROOT/.venv" ]; then
  _VENV_SRC="${VENV_DIR:-}"
  if [ -z "$_VENV_SRC" ] || [ ! -d "$_VENV_SRC" ]; then
    # $MAIN_REPO_ROOT comes from versions.sh and resolves the main clone even
    # when $ROOT is a per-experiment worktree.
    for _try in \
        "$MAIN_REPO_ROOT/.venv" \
        ${SHARED_VENV_DIR:+"$SHARED_VENV_DIR"}; do
  ...
  if [ -n "$_VENV_SRC" ] && [ -d "$_VENV_SRC" ]; then
    ...
    echo "ERROR: Cannot find a usable venv. Set VENV_DIR or run install_local.sh." >&2
    exit 1
  fi
fi
...
if [ -f "$_VERSIONED_VENV/.ready" ] && [ -x "$_VERSIONED_VENV/bin/python" ]; then
  ...
  # No versioned venv, but the venv resolved above already has vllm. Use it
  # as-is rather than rebuilding: experiment sandboxes have no network and no
  # writable $HOME, so an install here would fail and take the run with it.
  export VENV_DIR="$(cd "$ROOT/.venv" && pwd -P)"
  echo "launch_gemma: no versioned venv for $_VERSIONS_HASH —" \
       "using existing venv $VENV_DIR ($("$VENV_DIR/bin/python" -c \
       'import vllm; print("vllm", vllm.__version__)' 2>/dev/null || echo '?'))"
else
  echo "launch_gemma: versioned venv missing — building (first use for this version set)"
  # install_user_venv.sh is the builder that populates $BASE_VENV_ROOT/<hash>;
  # install_local.sh only builds $ROOT/.venv and would leave the path below empty.
  bash "$ROOT/install_user_venv.sh" --local
  export VENV_DIR="$_VERSIONED_VENV"
  ln -sfn "$VENV_DIR" "$ROOT/.venv"
fi
