...
source "$ROOT/versions.sh"   # version pins + $MAIN_REPO_ROOT
# Allow VENV_DIR / UV to be overridden by the environment (e.g. from the
# Slurm job wrapper which sets up a per-job isolated venv).  Otherwise resolve
# a venv relative to the repo in the same order launch_gemma.sh uses, with an
# optional out-of-repo $SHARED_VENV_DIR last.  $PYTHON is used by the package
# install step, which runs *before* launch_gemma.sh creates $ROOT/.venv, so an
# unusable default here breaks the run before vLLM is ever started.
if [ -z "${VENV_DIR:-}" ] || [ ! -x "${VENV_DIR}/bin/python" ]; then
    for _try in "$ROOT/.venv" "$MAIN_REPO_ROOT/.venv" \
                ${SHARED_VENV_DIR:+"$SHARED_VENV_DIR"}; do
        if [ -x "$_try/bin/python" ]; then VENV_DIR="$_try"; break; fi
    done
    unset _try
fi
export VENV_DIR
